...
class AdaptiveDiscretizationMB(Agent):
    ...
    def update_obs(self, obs, action, reward, newObs, timestep, info):
        ...
        if timestep != self.epLen - 1:
            ...
            new_obs_loc = np.argmin(np.max(np.abs(np.asarray(next_tree.state_leaves) - newObs), axis=1))
        ...
        if t >= 2**(self.split_threshold * active_node.depth):
            ...
            if timestep >= 1:
                _ = tree.tr_split_node(active_node, timestep, self.inherit_flag, self.epLen, self.tree_list[timestep-1])
            else:
                _ = tree.tr_split_node(active_node, timestep, self.inherit_flag, self.epLen, self.tree_list[timestep-1])
