...
class RideshareGraphEnvironment(gym.Env):
    ...
    def __init__(self, config=env_configs.rideshare_graph_default_config):
        ...
        self.num_transit = 0
        self.in_transit = [[0, 0] for _ in range(self.num_cars)]
    ...
    def fulfill_req(self, dispatch, source, sink):
        ...
        self.in_transit[self.num_transit] = [
            sink, 2 * np.round(self.lengths[source, sink])]
        self.num_transit += 1

    def step_in_transit(self):
        for i in range(self.num_transit):
            self.in_transit[i][1] -= 1
            # When the transit is complete
            if self.in_transit[i][1] < 1:
                # Make the car who completed transit available again
                transit_arrival = self.in_transit[i]
                self.state[transit_arrival[0]] += 1

                # Remove the car from the transit space
                self.in_transit[i] = [0, 0]
                self.num_transit -= 1
